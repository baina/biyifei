	...
	for f = 1, table.getn(flts) do
		...
		if code == 200 then
            ...
            if allprice then
				...
				local respbody = {};
				-- local hc = http:new()
				print("http://localhost:9001/data-pu/elong/" .. string.lower(org) .. "/" .. string.lower(dst) .. "/" .. string.lower(flts[f]) .. "/" .. t .. "/");
				local body, code, headers, status = http.request {
				-- local ok, code, headers, status, body = http.request {
					url = "http://localhost:9001/data-pu/elong/" .. string.lower(org) .. "/" .. string.lower(dst) .. "/" .. string.lower(flts[f]) .. "/" .. t .. "/",
					--- proxy = "http://127.0.0.1:8888",
					timeout = 3000,
					method = "POST", -- POST or GET
					-- add post content-type and cookie
					headers = { ["Content-Type"] = "application/json", ["Content-Length"] = string.len(JSON.encode(tmpres)) },
					-- body = formdata,
					source = ltn12.source.string(JSON.encode(tmpres)),

			...
			if not tmppri then
				...
				if tmppri ~= JSON.null and tmppri ~= nil then
					...
					if rbid[pcontent.flightline_id] ~= true then
						...
						local ok, err = red:hset('ota:' .. string.upper(ngx.var.org) .. ':' .. string.upper(ngx.var.dst) .. ':' .. string.lower(ngx.var.source), ngx.var.gdate .. '/' .. ngx.var.bdate .. '/', JSON.encode(tmppri))
						if ok then
							local result = {};
							result["resultCode"] = 0;
							result["priceSource"] = string.lower(ngx.var.source);
